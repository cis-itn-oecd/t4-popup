jQuery.fn.extend({
    showOecdPopup: function(options) {
            ...
            if (plugin.settings.booleanOnExit || plugin.settings.booleanOnClick || plugin.settings.booleanOnMinutes) {
                ...
                if (localStorage) {
                    ...
                    if (indexOf(plugin.settings.pagesList, plugin.settings.currentUrl) > -1) {
                        localStorage.setItem('popupundefined_pagevisited', '0');
                    } 
                }
                ...
                if (plugin.settings.booleanOnMinutes) {
                    /*if (localStorage) {
                        ...
                        if (localStorage.getItem('popup' + plugin.settings.contentID + '_minuteCount') !== null &&
                            ((d - dd) / 1000) > parseInt(plugin.settings.showOnMinutes)) {
                            plugin.show();
                        }
        ...
        plugin.cleanState = function() {
            localStorage.removeItem("popupundefined_pagevisited");
            deleteCookie(plugin.settings.session);
            deleteCookie(plugin.settings.cookie);
